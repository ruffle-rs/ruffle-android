...
public:
	...
	bool Open(const char *conf) {
		...
		fluid_settings_setstr(settings, const_cast < char* > ("audio.sample-format"), const_cast < char* > ("16bits"));

		if (synthsamplerate == 0) {
			synthsamplerate = 48000;
		}

        double gain = (double)synthgain;
        if (gain<0)
            gain = 0;
        ...
        //__android_log_print(ANDROID_LOG_INFO, "MagicBox", "synthsamplerate %d %g", synthsamplerate, gain);

		fluid_settings_setnum(settings,
                              const_cast <char*> ("synth.sample-rate"), (double)synthsamplerate);
		
		fluid_settings_setnum(settings,
                              const_cast <char*> ("synth.gain"), gain);
		
		fluid_settings_setstr(settings,
                              const_cast <char*> ("synth.reverb.active"), const_cast <char*> ("yes"));

		fluid_settings_setstr(settings,
                              const_cast <char*> ("synth.chorus.active"), const_cast <char*> ("yes"));

		fluid_settings_setnum(settings,
                              const_cast <char*> ("audio.periods"), 2);

		fluid_settings_setnum(settings,
                              const_cast <char*> ("audio.period-size"), 256);

		fluid_settings_setnum(settings,
                              const_cast <char*> ("player.reset-synth"), 0);

		fluid_settings_setnum(settings,
                              const_cast <char*> ("synth.min-note-length"), 0);

		fluid_settings_setstr(settings,
                              const_cast <char*> ("player.timing-source"), const_cast <char*> ("system"));

		fluid_settings_setnum(settings,
                              const_cast <char*> ("synth.cpu-cores"), 1);
		...
		fluid_settings_setstr(settings,
                              const_cast <char*> ("synth.midi-bank-select"), const_cast <char*> ("gs"));
