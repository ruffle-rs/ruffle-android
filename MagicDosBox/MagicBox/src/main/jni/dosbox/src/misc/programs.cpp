...
*/
...
#ifdef ANDROID
			//Section_prop* psec = (typeid(*sec) == typeid(Section_prop))? static_cast <Section_prop*>(sec) : NULL;
...
#ifdef ANDROID
				//Section_line* pline = (typeid(*sec) == typeid(Section_line))? static_cast <Section_line*>(sec) : NULL;
...
#ifdef ANDROID
							//Prop_int* pint = (typeid(*p) == typeid(Prop_int))? static_cast <Prop_int*>(p) : NULL;
		...
		case P_AUTOEXEC_CLEAR: {
			Section_line* sec = static_cast <Section_line*>(control->GetSection(std::string("autoexec")));
			sec->data.clear();
			break;
		}
		case P_AUTOEXEC_ADD: {
