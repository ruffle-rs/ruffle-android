...
//CatLog::log("localDrive::FileCreate %s", temp_name);	
	...
	{
		test = NULL;
		existing_file = Android_FileExists(temp_name);		
	}
	else
	{
...
#if defined (ANDROID)
	if (isSAF) {
		hand=Android_OpenFile(temp_name,"wb+");	
	} else {
	...
	*file=new localFile(this,name,hand);
...
//CatLog::log("localDrive::FileOpen %s", newname);
...
#if defined (ANDROID)
	if (isSAF) {
		hand=Android_OpenFile(newname,type);
	} else {
	...
	//CatLog::log("filefix localDrive::FileOpen %s, %s", newname, type);
...
//CatLog::log("localDrive::GetSystemFilePtr %s", name);
	char newname[CROSS_LEN];
	strcpy(newname,basedir);
	strcat(newname,name);
	CROSS_FILENAME(newname);
...
#if defined (ANDROID)
	if (isSAF) {
		return Android_OpenFile(newname,type);	
	}	
#endif
...
//CatLog::log("localDrive::GetSystemFilename %s %s", sysName, dosName);
...
//CatLog::log("localDrive::FileUnlink %s", name);
	char newname[CROSS_LEN];
	strcpy(newname,basedir);
	strcat(newname,name);
	CROSS_FILENAME(newname);
	...
	if (isSAF)
	{
		if (Android_deleteFile(fullname) == 0) 
		{
...
bool localDrive::FindFirst(char * _dir,DOS_DTA & dta,bool fcb_findfirst) {
  //CatLog::log("localDrive::FindFirst");
...
#if defined (ANDROID)	
	if (isSAF) {
		result = Android_stat(dirCache.GetExpandName(full_name),&stat_block);	
	} else {
		result = stat(dirCache.GetExpandName(full_name),&stat_block);
	}	
#else
    result = stat(dirCache.GetExpandName(full_name),&stat_block);
#endif	
...
//CatLog::log("localDrive::GetFileAttr %s", name);
	char newname[CROSS_LEN];
	strcpy(newname,basedir);
	strcat(newname,name);
	CROSS_FILENAME(newname);
	...
	if (isSAF) {
		result = Android_stat(newname,&status);	
	} else {
		result = stat(newname,&status);
	}
#else
	result = stat(newname,&status);
#endif	
	if (result==0) {
		*attr=DOS_ATTR_ARCHIVE;
		...
		//CatLog::log("localDrive::GetFileAttr true");
		return true;
	}
	//CatLog::log("localDrive::GetFileAttr false");
	*attr=0;
...
//CatLog::log("localDrive::MakeDir %s", dir);
	...
	if (isSAF) {
		temp=Android_MkDir(dirCache.GetExpandName(newdir));	
	} else {
		temp=mkdir(dirCache.GetExpandName(newdir),0700);
	}	
#else
	temp=mkdir(dirCache.GetExpandName(newdir),0700);
#endif
...
//CatLog::log("localDrive::RemoveDir %s", dir);  
	char newdir[CROSS_LEN];
	strcpy(newdir,basedir);
	strcat(newdir,dir);
	CROSS_FILENAME(newdir);
...
#if defined (ANDROID)
	if (isSAF) {
		temp=Android_RmDir(dirCache.GetExpandName(newdir));
	} else {
...
//CatLog::log("localDrive::TestDir %s", dir);  
	char newdir[CROSS_LEN];
	strcpy(newdir,basedir);
	strcat(newdir,dir);
	CROSS_FILENAME(newdir);
...
#if defined (ANDROID)	
		if (isSAF) {
			result = Android_stat(newdir,&test);
		} else {
			result = stat(newdir,&test);
		}
#else
		result = stat(newdir,&test);
#endif			
...
#if defined (ANDROID)	
	if (isSAF) {
		return Android_Access(newdir,F_OK)==0;
	}
#endif
...
//CatLog::log("localDrive::Rename %s %s", oldname, newname);  
...
#if defined (ANDROID)
	if (isSAF) {
		temp=Android_Rename(newold,dirCache.GetExpandName(newnew));	
	} else {
		temp=rename(newold,dirCache.GetExpandName(newnew));	
	}
#else
	temp=rename(newold,dirCache.GetExpandName(newnew));
#endif
...
#if defined (ANDROID)	
	if (isSAF) {
	    return Android_FileExists(newname);
	}
#endif	
...
bool localDrive::FileStat(const char* name, FileStat_Block * const stat_block) {
  //CatLog::log("localDrive::FileStat %s", name);  
	char newname[CROSS_LEN];
	strcpy(newname,basedir);
	strcat(newname,name);
	CROSS_FILENAME(newname);
...
#if defined (ANDROID)	
	if (isSAF) {
		result = Android_stat(newname,&temp_stat);
	} else {
		result = stat(newname,&temp_stat);
	}
#else
    result = stat(newname,&temp_stat);
#endif			
...
#if defined (ANDROID)
	isSAF = Android_IsSAFDrive(startdir);
#else
	isSAF = false;
#endif
	
	//CatLog::log("tento disk SAF==%d %s",(isSAF)?1:0, startdir);
}
...
bool localFile::Write(Bit8u * data,Bit16u * size) {
	...
	if(*size==0){  
	  ...
	  {
		  char newname[CROSS_LEN];
		  strcpy(newname,drive->basedir);
		  strcat(newname,name);
		  CROSS_FILENAME(newname);
	
	      fhandle = Android_FTruncate(fhandle, drive->dirCache.GetExpandName(newname), ftell(fhandle));
	      return true;
	  }
#endif
...
localFile::localFile(localDrive *_drive, const char *_name, FILE * handle) {
  //CatLog::log("filefix[drive_local.cpp] localFile::localFile name==%s", _name);
	drive=_drive;
	fhandle=handle;
	open=true;
