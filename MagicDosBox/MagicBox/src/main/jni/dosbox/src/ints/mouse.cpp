...
static bool maxEnabled;
//static bool microsoftMouse = false;
static int absoluteMouseType = 0;
static bool roundByVideoMode = false;
...
static struct {
	...
	Bit16s videoModeWidth, videoModeHeight;
	Bit16s int33MaxX, int33MaxY, int33PosX, int33PosY;
	Bit16s int33MaxXOrig, int33MaxYOrig;
...
void DrawCursor()
{
	...
	if (real_readb(BIOSMEM_SEG,BIOSMEM_CURRENT_PAGE)!=mouse.page)
	{
		return;
	}
...
void Mouse_CursorMoved(float xrel,float yrel,float x,float y,bool emulate) {
	...
	if (emulate) {
		...
		//CatLog::log("emulate relXY[%f,%f], XY[%f, %f], dXY[%f, %f]", xrel, yrel, mouse.x, mouse.y, dx, dy);
	} else {
		if (CurMode->type == M_TEXT) {
			mouse.x = x*CurMode->swidth;
			mouse.y = y*CurMode->sheight * 8 / CurMode->cheight;
		}
		else if ((mouse.max_x < 8192) || (mouse.max_y < 8192) || (mouse.max_x != mouse.max_y))
		{
			//CatLog::log("1");
			if ((mouse.max_x > 0) && (mouse.max_y > 0))
			{
				if (absoluteMouseType == 2) {
					//__android_log_print(ANDROID_LOG_INFO, "MagicBox", "2 oldxy[%f,%f] [%f,%f]", (float)mouse.x, (float)mouse.y, (float)x, (float)y);

					//2840, 2724
					mouse.x = mouse.max_x/2 + x * maxWidth;
					mouse.y = mouse.max_y/2 + y * maxHeight;
				} else {
...
void Mouse_NewVideoMode(void) {
	...
	//CatLog::log("c++Moved Mouse_NewVideoMode - %X", mode);
	...
	switch (mode)
	{
		...
		default:
			//CatLog::log("Unhandled videomode %X on reset", mode);
			//LOG(LOG_MOUSE,LOG_ERROR)("Unhandled videomode %X on reset",mode);
			mouse.inhibit_draw=true;
			return;
	}

	mouse.videoModeWidth = mouse.max_x + 1;
	mouse.videoModeHeight = mouse.max_y + 1;

	mouse.int33MaxX = 0;
	mouse.int33MaxY = 0;
	mouse.int33PosX = 0;
	mouse.int33PosY = 0;
	mouse.int33MaxXOrig = 0;
	mouse.int33MaxYOrig = 0;

	//CatLog::log("videomode %X,%d,%d", mode, mouse.videoModeWidth, mouse.videoModeHeight);

	Mouse_SetMouseMax(maxEnabled, maxWidth, maxHeight);
	...
	#if defined (ANDROID)
	Android_MouseSetVideoMode(mouse.mode, mouse.max_x, mouse.max_y);
	#endif
}
...
int Mouse_GetVideoWidth()
{
	return mouse.videoModeWidth;
}
int Mouse_GetVideoHeight()
{
	return mouse.videoModeHeight;
}

int Mouse_GetMaxX()
{
	if(absoluteMouseType == 2 && mouse.int33PosX > 0) {
		return mouse.int33PosX;
	}
...
int Mouse_GetMaxY()
{
	if(absoluteMouseType == 2 && mouse.int33PosY > 0) {
		return mouse.int33PosY;
	}

	return mouse.max_y;
}

bool Mouse_isMicrosoftMouse()
{
	//return microsoftMouse;
	return absoluteMouseType == 1;
}

void Mouse_SetAbsoluteType(int type)
{
	absoluteMouseType = type;
}

void Mouse_SetRoundMaxByVideMode(bool enabled)
{
	roundByVideoMode = enabled;
	...
	if (enabled)
	{
		if (mouse.int33MaxX > 0)
		{			
			mouse.max_x = mouse.int33MaxX;
		}

		if (mouse.int33MaxY > 0)
		{
			mouse.max_y = mouse.int33MaxY;
		}
	}
	else
	{
		if (mouse.int33MaxXOrig > 0)
		{
			mouse.max_x = mouse.int33MaxXOrig;
		}

		if (mouse.int33MaxYOrig > 0)
		{
			mouse.max_y = mouse.int33MaxYOrig;
		}
	}
}

void Mouse_SetMouseMax(bool enabled, int max_width, int max_height)
{
	//CatLog::log("Mouse_SetMouseScale e/w/h : %d/%d/%d", (enabled)?1:0, max_width, max_height);
	...
	{
		if (absoluteMouseType != 2) {
			mouse.max_x = (Bit16s) max_width;
			mouse.max_y = (Bit16s) max_height;
		}

		//CatLog::log("Mouse_SetMouseScale ENABLED maxX,Y[%d, %d]", mouse.max_x, mouse.max_y);
	}
	else
	{
		//CatLog::log("Mouse_SetMouseScale DISABLED int33[%d, %d] vm[%d, %d]", mouse.int33MaxX, mouse.int33MaxY, mouse.videoModeWidth, mouse.videoModeHeight);
		...
		{
			mouse.max_x = mouse.videoModeWidth - 1;
		}
		...
		{
			mouse.max_y = mouse.videoModeHeight - 1;
		}

		//CatLog::log("Mouse_SetMouseScale DISABLED maxX,Y[%d, %d]", mouse.max_x, mouse.max_y);
	}

	//mouse.max_x = mouse.max_x - 1;
	//mouse.max_y = mouse.max_y - 1;

	//CatLog::log("Mouse_SetMouseScale maxX,Y[%d, %d]", mouse.max_x, mouse.max_y);
}

static Bit16s roundResolution(float size, float max)
{
//disabled for now
/*
	float a = max;

	max = ((Bit16s)((max / size) + 0.05f)) * size;
	//max = (Bit16s)(floor(((max / size) + 0.05f) * 10) / 10 * size);

	//CatLog::log("roundResolution size, max [%f, %f, %d]", size, a, (Bit16s)max);
...
static Bitu INT33_Handler(void) {
	//CatLog::log("INT33_Handler : %04X %X %X %d %d",reg_ax,reg_bx,reg_cx,POS_X,POS_Y);
	switch (reg_ax) {
	...
	case 0x07:
		{
			//CatLog::log("INT33_Handler : %04X %X %X %d %d %d",reg_ax,reg_bx,reg_cx,reg_dx,POS_X,POS_Y);
			...
			mouse.int33PosX = POS_X;
			mouse.int33PosY = POS_Y;
			...
			mouse.min_x=min;
			mouse.int33MaxXOrig = max;
			mouse.int33MaxX = roundResolution(mouse.videoModeWidth, max) - 1;
			...
			//CatLog::log("Define horizontal range2 min:%d max:%d",min,max);
			
			//Or alternatively this:
			//mouse.x = (mouse.max_x - mouse.min_x + 1)/2;

			//LOG(LOG_MOUSE,LOG_NORMAL)("Define Hortizontal range min:%d max:%d",min,max);
			//CatLog::log("Define Horizontal range min:%d max:%d",min,max);
			break;
		}
	case 0x08:
		{
			...
			mouse.min_y=min;
			mouse.int33MaxYOrig = max;
			mouse.int33MaxY = roundResolution(mouse.videoModeHeight, max) - 1;
			...
			//CatLog::log("Define Vertical range2 min:%d max:%d",min,max);
			
			///Or alternatively this:
			//mouse.y = (mouse.max_y - mouse.min_y + 1)/2;

			//LOG(LOG_MOUSE,LOG_NORMAL)("Define Vertical range min:%d max:%d",min,max);
			//CatLog::log("Define Vertical range min:%d max:%d",min,max);
			break;
		}
	...
	default:
		...
		//CatLog::log("Mouse Function %04X not implemented!",reg_ax);
		break;
	}
	return CBRET_NONE;
}
...
static Bitu INT74_Handler(void) {
	//CatLog::log("INT74_Handler : %04X %X %X %d %d",reg_ax,reg_bx,reg_cx,POS_X,POS_Y);
	if (mouse.events>0) {
		mouse.events--;
