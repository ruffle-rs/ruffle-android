...
//		LOG_MSG("reset by time %d %d",PIC_Ticks,last_write);
	...
	**/
	...
	if (stick[0].enabled) {
		...
		//20160511 - Bruenor - 2axis type and 4 buttons support
		if (stick[0].button[2]) ret&=~64;
		if (stick[0].button[3]) ret&=~128;
	}
	if (stick[1].enabled) {
...
void JOYSTICK_Button(Bitu which,Bitu num,bool pressed) {
	//20160511 - Bruenor - 2axis type and 4 buttons support
	//changed from (num<2) to (num<4)
	if ((which<2) && (num<4)) stick[which].button[num]=pressed;
}
...
bool JOYSTICK_GetButton(Bitu which, Bitu num) {
	//20160511 - Bruenor - 2axis type and 4 buttons support
	//changed from (num<2) to (num<4)
	if ((which<2) && (num<4)) return stick[which].button[num];
	return false;
}
